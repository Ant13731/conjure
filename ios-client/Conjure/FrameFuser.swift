...
actor FrameFuser {
    ...
    private func tryFuse(at ts: Int) async {
        ...
        for landmark in awaited_landmarks {
            ...
            landmarks.append(Landmark(x: landmark.x, y: landmark.y, z: depth, visibility: landmark.visibility as? Float))
        }

        let frame = Frame(handedness: handedness, gesture: gesture, handedness_confidence: handedness_confidence, gesture_confidence: gesture_confidence, timestamp: ts, landmarks:landmarks)
